...
/**
 * @author Reese Wells 1. Imports voter files 2. Imports verification files 3.
 *         Verifies the data 4. Saves the data
 */
public class DataHandler {
	// the arrays for saving the data
	private static ArrayList<String[]> dataArray = new ArrayList<String[]>(); // holds
																				// the
																				// districtData
																				// array
	private static ArrayList<String[]> countyArray = new ArrayList<String[]>(); // holds
																				// countyVotes
																				// array
	private static ArrayList<String[]> registeredCountyArray = new ArrayList<String[]>(); // holds
																							// the
																							// registeredCountyVotes
																							// array
	...
	static String name = "console_log_" + LocalDate.now() + "_" + LocalTime.now().getHour() + "_"
			+ LocalTime.now().getMinute() + ".txt";
	...
	public static void main(String[] args) {
		...
		if (errors.size() > 0) {
			JOptionPane.showMessageDialog(null,
					"Erros were found, the log file has been placed in the folder with the program");
		}
	...
	public static State makeState(String directory) {
		...
		if (errors.size() > 0) {
			JOptionPane.showMessageDialog(null,
					"Erros were found, the log file has been placed in the file with the program");
		}
			...
			for (int i = 0; i < dataArray.size(); i++) {
				for (int k = 0; k < registeredCountyArray.size(); k++) {
					if (countyArray.get(i)[COUNTY_NAME].equals(registeredCountyArray.get(k)[COUNTY_NAME])) {
						found = true;
						if (Integer.parseInt(countyArray.get(i)[VOTE_COUNT]) > Integer
								.parseInt((registeredCountyArray.get(k)[VOTE_COUNT]))) {
							overVoteCount = true;
						}
					}
				}
				...
				if (!found) {
					out.println("removing " + Arrays.toString(dataArray.get(i))
							+ ". Not found in registered counties file.");
					remove.add(i);
				}

				if (overVoteCount) {
					out.println("removing " + Arrays.toString(dataArray.get(i)) + ". Vote count was "
							+ countyArray.get(i)[VOTE_COUNT] + ", should have been "
							+ registeredCountyArray.get(i)[VOTE_COUNT]);
					remove.add(i);
				}
	...
	private static String getFiles() {
		...
		if (result == JFileChooser.APPROVE_OPTION) {
			String path = fc.getSelectedFile().getAbsolutePath().toString(); // returns
																				// a
																				// string
																				// of
																				// the
	...
	private static ArrayList<String[]> getDistrictData(String path) {
		try {
			...
			while (checkIn.hasNextLine()) {
				...
				if (fixedLine.length == 5 && fixedLine[2].matches("\\d+") && fixedLine[3].matches("\\d+")
						&& fixedLine[4].matches("\\d+")) {
					System.out.println(Arrays.toString(fixedLine));
					hasData = true;
					break;
				}
	...
	/**
	 * @param num
	 * @return int removes letters mixed in with numbers
	 */
	...
	private static void err(int error, String reference) {
		...
		switch (error) {
		case EXTRACTION:
			message = "Extract error at " + reference;
			break;
		case FILE_NOT_FOUND:
			message = "File not found at " + reference;
			break;
		case DATA_FORMAT:
			message = "data format error at " + reference;
			break;
		case DUPLICATE:
			message = "duplicate region found at " + reference;
			break;
		case TOO_MANY_VOTES:
			message = "too many voters found at " + reference;
			break;
		case NEGATIVE_VOTES:
			message = "negative voters found at " + reference;
			break;
		case MISSING_VALUE:
			message = "missing value at " + reference;
			break;
		case EXTRA_SYMBOL:
			message = "extra symbols found at " + reference;
			break;
		case CORRUPT:
			message = "data is corrupt at " + reference;
			break;
		case TAMPER:
			message = "data has been tampered with at " + reference;
			break;
		default:
			message = "error not recognized, error number " + Integer.toString(error);
		}
		out.write(message);
		out.println();
		out.flush();
